...
pub fn download(url: &str, destination: &Path, expected_sha256: Option<&str>) -> Result<DownloadResult, String> {
    ...
    let client = Client::builder().timeout(Duration::from_secs(3600)).user_agent("LocalAI-Studio/1.0").build()
    ...
    let mut file = if resumed {
        let mut f = OpenOptions::new().create(true).append(true).open(&temp)
    ...
    if let Some(expected) = expected_sha256 {
        if !expected.eq_ignore_ascii_case(&sha256) { return Err(format!("اعتبارسنجی SHA-256 شکست خورد؛ مقدار دریافت‌شده: {sha256}")); }
    }
